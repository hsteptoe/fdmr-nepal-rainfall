...
```{r}
nepal_data <- qread('nepal-data.qs')
```

```{r}
min_cyear <- min(nepal_data$cyear) - 1 # find the minimum year count
nepal_data <- transform(nepal_data, year = as.Date(ISOdate(year, 1, 1))) # convert 'year' to date format
nepal_data <- transform(nepal_data, cyear = cyear - (min_cyear)) # set the year count to start from 1
```
...
## Subset the Dataset here

```{r}
# nepal_data <- nepal_data[1:100,,drop=FALSE]
nepal_data <- as.tibble(nepal_data) %>% filter(model == 'MSWEP')
```
...
```{r}
start_year <- c(sort(unique(nepal_data$year)))[1]
end_year <- c(sort(unique(nepal_data$year)))[length(unique(nepal_data$year))]
```
We plot the yearly sum of reported rainfall spanning years available:

```{r plotcases, error=TRUE, fig.cap="The yearly sum of reported rainfall spanning years available", fig.align = "center"}
breaks_vec <- c(seq(start_year, end_year, by = "1 year"), end_year)

rainfall_year <- dplyr::group_by(nepal_data, year) %>% dplyr::summarize(prcp = sum(prcp))

fdmr::plot_barchart(data = rainfall_year, x = rainfall_year$year, y = rainfall_year$prcp, breaks = breaks_vec, x_label = "Year", y_label = "Rainfall sum")
```
...
proj4string(nepal_data) <- CRS("+init=epsg:4326")
```
...
formula <- prcp ~ 0 + Intercept(1) + f(main = coordinates,
    model = matern, group = cyear)
```
...
```{r}
fit <- bru(formula, family="gaussian", data=nepal_data, options=list(bru_verbose=4,verbose=TRUE))
```
